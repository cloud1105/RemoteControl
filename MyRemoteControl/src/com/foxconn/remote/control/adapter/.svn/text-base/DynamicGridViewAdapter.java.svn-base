 ...
 * 
 */
public class DynamicGridViewAdapter extends BaseDynamicGridAdapter implements
	...
	public DynamicGridViewAdapter(Context context, List<?> items,
			int columnCount) {
	...
	public int getCount() {
		/**
		 * 1.编辑模式下最后一个View为加号时隐藏最后的加号 
		 * 2.最后一个View填满了且最后一个显示的View不为加号时隐藏最后的加号
		 * 3.其他情况显示最后的加号
		 */
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final Machine selItem = (Machine) getItem(position);
...
//				holder.build("+", adap);
			...
			holder.build(selItem.getMachineName(), selItem.getImg());
			/**
